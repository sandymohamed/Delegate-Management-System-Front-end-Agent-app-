...
import {
  ...
  Paper, Grid2, InputAdornment, Stack, Table, TableBody,
  ...
  return (
    <Container >
      <Box>
        ...
        <Stack direction="row" spacing={2} alignItems="center" justifyContent="space-between">
          <Box sx={{ display: 'flex', alignItems: 'center' }}>
            <Typography gutterBottom>  عدد النتجات : {totalDataLength} </Typography>
          ...
          <Box sx={{ display: 'flex', alignItems: 'center', gap: 2 }}>
            <TextField
              ...
              variant="outlined"
            />
            <Button variant='contained' color='secondary' component={RouterLink} to={`/products/create-product`}> اضافة منتج جديد</Button>
          </Box>
        ...
        <Grid2 container spacing={3} >
          {!tableTheme ? TableData?.map((product) => (
            <Grid2 item size={{ xs: 6, sm: 3, md: 2 }} key={product?.id}>
              <Card>
              ...
              <Table stickyHeader aria-label="sticky table">
                <TableHead>
                  <TableHeadRow >
                    ...
                    <TableHeadCell title='تاريخ اقرب كرتونة'> تاريخ انتهاء الصلاحية </TableHeadCell>
                ...
                <TableBody>
                  {TableData?.map((product) => (
                    ...
                    >
                      ...
                      <TableBodyCell component="th" scope="row" >{product.name}</TableBodyCell>
      ...
      {(TableData?.length && !tableTheme) &&
        <Box sx={{ display: 'flex', justifyContent: 'end', alignItems: 'bottom', mt: 3 }}>
          <IconButton color='secondary' aria-label="next" size="small" onClick={() => setPage(page + 1)} disabled={totalDataLength <= (page + 1) * rowsPerPage} >
            <Iconify icon={icons.back} width={24} height={24} /> التالى
          </IconButton>
        </Box>
      }
    </Container>
